...
export const mockHabitsToday: TodayHabit[] = [
  { id: 1, name: "Drink Water", time: "8:00 AM" },
  { id: 2, name: "Exercise", time: "9:00 AM" },
  { id: 3, name: "Read a Book", time: "10:00 AM" },
  { id: 4, name: "Meditate", time: "11:00 AM" },
  { id: 5, name: "Sleep Early", time: "12:00 PM" },
  { id: 6, name: "Extra Extra Extra Extra Extra Long Text Test", time: "1:00 PM" },
  { id: 7, name: "Test", time: "2:00 PM" },
  { id: 8, name: "Test", time: "3:00 PM" },
  { id: 9, name: "Test", time: "4:00 PM" },
  { id: 10, name: "Test", time: "5:00 PM" },
];
...
export const initialHabits: Habit[] = [
  ...
  { id: 6, name: "Extra Extra Extra Extra Extra Long Text Test Test Test Test Test tEst Test" },
  { id: 7, name: "Test" },
  { id: 8, name: "Test" },
  { id: 9, name: "Test" },
  { id: 10, name: "Test" },
...
export const useHabits = (initial: Habit[] = []) => {
  ...
  return { habits, addHabit, deleteHabit, editHabit, reorderHabits, setHabits } as const;
};
...
export const useTodaysHabits = () => {
  ...
  const sorted = useMemo(() => {
    return [...todayHabits].sort((a, b) => parseTimeToMinutes(a.time) - parseTimeToMinutes(b.time));
  }, [todayHabits]);

  const addTodayHabit = (habit: TodayHabit) => setTodayHabits((prev) => [...prev, habit]);
  const deleteTodayHabit = (id: number) => setTodayHabits((prev) => prev.filter((h) => h.id !== id));
  const editTodayHabit = (id: number, updates: Partial<TodayHabit>) =>
    setTodayHabits((prev) => prev.map((h) => (h.id === id ? { ...h, ...updates } : h)));

  return { todayHabits, sorted, addTodayHabit, deleteTodayHabit, editTodayHabit, setTodayHabits } as const;
};